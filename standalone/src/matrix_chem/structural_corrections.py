...
CV_RADIAL_BOND_SCALE_PERIOD_LINE = (-2.5992083901, 0.2652521252)
...
def cv_radial_bond_scale_delta_milliangstrom(
    atomic_number_a: int, atomic_number_b: int
) -> float | None:
    za, zb = int(atomic_number_a), int(atomic_number_b)
    radius_a = CV_RADIAL_COVALENT_RADII_ANGSTROM.get(za)
    radius_b = CV_RADIAL_COVALENT_RADII_ANGSTROM.get(zb)
    period_a, period_b = CV_RADIAL_PERIOD.get(za), CV_RADIAL_PERIOD.get(zb)
    valence_a = CV_RADIAL_VALENCE_ELECTRONS.get(za)
    valence_b = CV_RADIAL_VALENCE_ELECTRONS.get(zb)
    if None in {radius_a, radius_b, period_a, period_b, valence_a, valence_b}:
        return None
    gamma0, gamma1 = CV_RADIAL_BOND_SCALE_PERIOD_LINE
    n_a, n_b = min(int(period_a), 3), min(int(period_b), 3)
    return float(radius_a + radius_b) * np.sqrt(n_a * n_b - 1.0) * (
        gamma0 + gamma1 * max(int(valence_a), int(valence_b))
    )
...
def cv_radial_posterior_amplitude_milliangstrom(
    ...
    atomic_number_b: int,
    *,
    model: str = "radius-aware-period-line",
) -> float | None:
    normalized = str(model).strip().lower().replace("_", "-")
    if normalized in {"radius-aware", "radius-aware-period-line", "atomic-radius-aware"}:
    ...
    if normalized in {"bond-scale", "bond-scale-period-line"}:
        return cv_radial_bond_scale_delta_milliangstrom(atomic_number_a, atomic_number_b)
    raise ValueError(f"unknown CV-radial amplitude model: {model}")
...
def cv_radial_bond_delta_angstrom(
    ...
    distance_angstrom: float,
    *,
    amplitude_model: str = "radius-aware-period-line",
    ...
    if weight < weight_threshold:
        return None
    amplitude = cv_radial_posterior_amplitude_milliangstrom(
        atomic_number_a, atomic_number_b, model=amplitude_model
    )
...
def perceive_hydrogen_bonds(
    ...
    *,
    ...
    """
    from .topology.vdw_radii import vdw_radius
    ...
    for hydrogen, z_h in enumerate(numbers):
        ...
        for acceptor, z_acceptor in enumerate(numbers):
            ...
            if tuple(sorted((hydrogen, acceptor))) in bonds:
                continue
            radius_h = vdw_radius(1, scheme="uff")
            radius_a = vdw_radius(z_acceptor, scheme="uff")
