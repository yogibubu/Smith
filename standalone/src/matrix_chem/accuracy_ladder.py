...
def build_accuracy_ladder_plan(
    ...
    *,
    ...
    cv_amplitude_model: str = "radius-aware-period-line",
    ...
    if include_core_valence:
        for index, primitive in enumerate(primitives):
            ...
            delta = core_valence_bond_shift(
                ...
                distance,
                amplitude_model=cv_amplitude_model,
    ...
    if include_bl1_conjugation or include_pl1_hydrogen_bonds:
        ...
        targets.extend(
            build_l1_refinement_targets(
                ...
                cv_amplitude_model=cv_amplitude_model,
            )
        )
...
def _conjugation_delta_angstrom(
    ...
    from .topology.pykko_radii import bond_multiplicity_reference_radii

    try:
        left = bond_multiplicity_reference_radii(int(z_left), fallback_radius=0.0)
        right = bond_multiplicity_reference_radii(int(z_right), fallback_radius=0.0)
    except ValueError:
        return 0.0
...
def _principal_quantum_number(atomic_number: int) -> int:
    z = int(atomic_number)
    if z < 1 or z > 118:
        raise ValueError(f"invalid atomic number: {atomic_number}")
    for shell, upper in enumerate((2, 10, 18, 36, 54, 86, 118), start=1):
        if z <= upper:
            return shell
