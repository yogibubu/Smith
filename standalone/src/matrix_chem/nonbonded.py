...
from .topology.vdw_radii import uff_well_depth_kcal, vdw_radius
...
def preferred_atomic_charges_from_xyzin(
    ...
    *,
    ...
    """Choose charges for MM subtraction, preferring complete CM5 vectors.

    ``auto`` ranks CM5 above other models and, within the same model class,
    charges carried by ``#CARTESIAN_HESSIAN`` above the frozen synthon vector.
    Explicit ``hessian`` and ``synthons`` modes are available for reproducible
    overrides.
    """
    normalized = str(preference).strip().lower()
    if normalized not in {"auto", "hessian", "synthons"}:
        raise ValueError("charge preference must be auto, hessian or synthons")

    candidates: list[tuple[np.ndarray, str, bool, bool]] = []
    if hessian_charges is not None:
        ...
        if values.size:
            ...
            candidates.append((values, f"#CARTESIAN_HESSIAN {model}", True, "CM5" in model.upper()))

    try:
        synthon_values, synthon_source = synthon_charges_from_xyzin(path, natoms)
    except ValueError:
        synthon_values = None
    if synthon_values is not None:
        candidates.append(
            (
                synthon_values,
                f"#SYNTHONS {synthon_source}",
                False,
                "CM5" in synthon_source.upper(),
            )
        )

    if normalized == "hessian":
        candidates = [candidate for candidate in candidates if candidate[2]]
    elif normalized == "synthons":
        candidates = [candidate for candidate in candidates if not candidate[2]]
    if not candidates:
        raise ValueError(f"no complete atomic charge vector is available for {normalized}")
    if normalized == "auto":
        candidates.sort(key=lambda candidate: (not candidate[3], not candidate[2]))
    values, source, _from_hessian, _is_cm5 = candidates[0]
