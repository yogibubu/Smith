...
def write_topology_section(
    ...
    *,
    ...
    bond_order_source = getattr(synthons, "_bond_order_source", "ORACLE continuous topology model")
    fh.write(f"BOND_ORDER_SOURCE {bond_order_source}\n")
    multiplicity_source = getattr(
        synthons,
        "_bond_multiplicity_source",
        "ORACLE Pauling multiplicity model (Mantina-Truhlar radii; decay 0.30 A)",
    )
    fh.write(f"BOND_MULTIPLICITY_SOURCE {multiplicity_source}\n")
    ...
    fh.write(
        "COLUMNS ATOM_I ATOM_J BO_TOPOLOGY BO_MULTIPLICITY "
        "BO_SIGMA BO_PI BO_PI_PI\n"
    )
    component_rows = []
    if synthons is not None:
        for i, j in dg.bonds:
            try:
                ...
                component_rows.append(
                    f"{i + 1} {j + 1} {float(synthons.bond_order(i, j)):.10g} "
                    f"{components.total:.10g} "
