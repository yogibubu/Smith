...
def _parse_bond_order_components(
    ...
    *,
    ...
    for line in _subsection(topology, "BOND_ORDER_COMPONENTS"):
        ...
        if len(numeric) == 5:
            topology_order, multiplicity, sigma, pi, pi_pi = numeric
        else:
            topology_order = multiplicity = numeric[0]
            sigma, pi, pi_pi = numeric[1:]
        rows.append(
            {
                "atoms": tuple(sorted((i, j))),
                "topology": round(topology_order, rounding_decimals),
                "multiplicity": round(multiplicity, rounding_decimals),
