...
def print_topology_report(
    ...
    *,
    ...
    with open(outfile, "w") as fh:
        ...
        multiplicity_source = getattr(synthons, "_bond_multiplicity_source", "ORACLE Pauling")

        fh.write("\nTOPOLOGY INPUT SOURCES\n")
        fh.write("----------------------\n")
        fh.write(f"Atomic charges source: {charge_source}\n")
        fh.write(f"Topological edge-order source: {bo_source}\n")
        fh.write(f"Bond-multiplicity source: {multiplicity_source}\n")
        ...
        for i, j in dg.bonds:
            ...
            multiplicity = synthons.bond_order_multiplicity(i, j)
            sigma, pi, pi_pi = synthons.bond_order_indices(i, j)

            fh.write(
                f"({i + 1:2d},{j + 1:2d})  {zi:>2s}–{zj:<2s}   r = {rij:6.3f} Å   "
                f"BO_topo = {bo:6.3f}  M = {multiplicity:6.3f}  "
