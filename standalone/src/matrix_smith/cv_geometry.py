...
from matrix_chem.structural_corrections import (
    CV_RADIAL_BOND_SCALE_PERIOD_LINE as ORACLE_CV_RADIAL_BOND_SCALE_PERIOD_LINE,
    ...
    cv_radial_bond_scale_delta_milliangstrom as oracle_cv_radial_bond_scale_delta_milliangstrom,
...
CV_RADIAL_RADIUS_AWARE_PERIOD_LINE = {
    2: (-4.2017837543, 0.5303223136),
    3: (-6.8796506863, 0.7358761866),
}

CV_RADIAL_BOND_SCALE_PERIOD_LINE = (-2.4210492653, 0.2359712865)
...
# Compatibility exports: SMITH/GF still import these historical names, but the
# calibration and equations are now owned by ORACLE/matrix-chem.
...
CV_RADIAL_BOND_SCALE_PERIOD_LINE = ORACLE_CV_RADIAL_BOND_SCALE_PERIOD_LINE
...
def apply_cv_radial_geometry_correction(
    ...
    *,
    ...
    corrected: list[tuple[int, float, str]] = []
    for index, primitive in enumerate(definition.primitives):
        ...
        corrected.append((index, delta, amplitude_model))
    ...
    residuals = [float(final_values[index] - target_values[index]) for index, _delta, _source in corrected]
    sources = {source for _index, _delta, source in corrected}
    ...
    return corrected_coords, CVRadialGeometryCorrection(
        ...
        max_abs_delta_angstrom=max(abs(delta) for _index, delta, _source in corrected),
...
def cv_radial_posterior_amplitude_milliangstrom(
    ...
    *,
    ...
    return oracle_cv_radial_posterior_amplitude_milliangstrom(
        atomic_number_a, atomic_number_b, model=model
    )
...
def cv_radial_bond_scale_period_line_delta_milliangstrom(
    atomic_number_a: int,
    atomic_number_b: int,
) -> float | None:
    return oracle_cv_radial_bond_scale_delta_milliangstrom(
