...
The name \NEO\ uses ``equivariant'' in the classical molecular-symmetry sense:
point-group operations map coordinate rows into the same space with the
corresponding atom permutation and vector or pseudovector signs.  It does not
denote a machine-learning model, nor a nuclear-electronic-orbital method.
``Orthogonalizer'' refers to normalized Wilson-row rank selection and
orthonormal SALC coefficient vectors inside homogeneous blocks, not to global
orthogonality in the kinetic \(\Gmat\) metric.
...
synthon descriptors, and \NEO\ uses them to make reproducible local choices.
...
a family are resolved by topology, \ORACLE\ equivalence and
