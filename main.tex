...
\newcommand{\NEO}{\SONIC}
\newcommand{\TheONE}{\textsc{The\_ONE}}
...
\TheONE\ is the standalone tool that constructs \SONIC\ contracts, while
...
\TheONE{}--\SONIC & Coordinate-construction tool and its coordinate
...
\end{tikzpicture}
\caption{Constructive workflow implemented by \TheONE\ for \SONIC\ coordinates.
...
contracts.  The standalone command-line tool is named \TheONE; legacy
...
\TheONE\ TOML input file with schema \texttt{matrix.the\_one.input.v1}.  The file
contains one \texttt{[geometry]} table, either as an external coordinate path or
as inline Cartesian coordinates, and one options table controlling
symmetrization, \texttt{SYCART} export, fragment treatment, X--H policies,
improper-dihedral compatibility and an optional reduced working subgroup.
Thus a distributable input is one file with coordinates and construction
options, while the generated \texttt{xyzin} remains the frozen audited state.
...
is needed when the coordinate problem is simple; camphor adds a larger
bridged-ring case with 75 vibrational degrees of freedom.  These calculations
are not a benchmark of electronic
structure theory or a claim that one coordinate choice always minimizes the
iteration count.  They test the narrower point that \TheONE\ can generate a
Gaussian-readable \SONIC\ coordinate model without manual GIC transcription and
that Gaussian can use it with the same convergence behavior as built-in
coordinate choices.  At HF/STO-3G, water converges in four steps with default,
Cartesian, hand Z-matrix and \SONIC\ \texttt{ReadAllGIC} coordinates.  Hydrogen
peroxide converges in six steps with Gaussian default, Cartesian and \SONIC\
coordinates and in five steps with the hand Z matrix.  For camphor the
automatically generated \SONIC\ block converges in eight steps, compared with
14 Cartesian steps and 30 steps for a sequential Z matrix; the Gaussian DV
redundant-internal default did not complete from this generated Cartesian input
in the present run.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\linewidth]{figures/gaussian_coordinate_comparison.png}
\caption{Gaussian DV HF/STO-3G optimization-step comparison for built-in
...
portability; camphor probes a larger bridged-ring case where a naive Z matrix
...
generated with the \TheONE\ development tree for \SONIC\ coordinates and
...
used to generate the tables and application plots.  The \TheONE\ source
