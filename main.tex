...
force-field construction, PES fitting and semi-experimental least-squares
refinement.  A separate functional layer can then evaluate inverse or
...
models.  TRINITY is the future external-gradient optimization consumer of the
same GIC/SYCART state.  SMITH (Surface Modeling with Internal-coordinate Terms
and Hamiltonians) is the planned consumer for multidimensional potential-energy
and property-surface fitting, while MORPHEUS uses the frozen basis for one- and
multidimensional semi-experimental fits.  In all cases the scientific object is
the same: a rank-complete coordinate basis with a known Wilson matrix, symmetry
...
\(\phi\) descriptors derived from linear \(RPck\) components.  The active
Gaussian-readable coordinate basis should remain linear and rank complete,
while such nonlinear functions become named observables or model variables in
GF/PED, future TRINITY and SMITH workflows, or MORPHEUS.
...
future TRINITY optimization, future SMITH PES fitting, MORPHEUS
semi-experimental refinement and workflow diagnostics.  Nonlinear coordinate
functions such as inverse or exponential distances, trigonometric angular terms
and Cremer--Pople descriptors can be built on top of this base by analytic
