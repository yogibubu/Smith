...
Figure~\ref{fig:coordinate-atlas} anchors the construction in four recurring
geometrical motifs.  Ordinary stretches and bends, curvilinear ring
out-of-plane sources, fused-ring puckering/butterfly rows and relative
fragment pose coordinates enter different homogeneous source families.  Their
separation is preserved during reduction even when two rows carry the same
global irreducible-representation label.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/sonic_coordinate_atlas.png}
\caption{Visual atlas of representative \SONIC\ source families, drawn on the
actual molecular geometries used by the implementation fixtures.  Amber marks
stretching or butterfly-like displacement, teal a valence-angle or weak-contact
relation, violet the signed curvilinear out-of-plane/puckering family, and blue
relative fragment translation.  The overlays identify coordinate differentials;
they are not a prescription for a finite internal-to-Cartesian step.}
...
given in Supporting Information Sec.~S1.  Thus SMITH is operationally
...
\caption{Structures used to exercise protected special coordinates.  Diamonds
denote stored ring, ligand or fragment centers; dashed teal segments denote
center--atom or selected contact coordinates; blue elements denote relative
fragment pose.  Ferrocene uses two Cp centers, formic-acid--water and the water
dimer use disconnected-fragment rows, benzene--water combines a ring center
with a fragment pose, and $\eta^3$ allyl--PdCl retains the supplied allyl center
without introducing a fictitious atom into the covalent topology.}
...
Table~\ref{tab:frisch-si} gives a representative validation slice aimed directly at
...
block-local multi-\(U\) \(RPck\) coordinates.  Because commercial Gaussian 16
...
corresponding commercial-Gaussian input is necessarily less compact because
