...
rank-complete, symmetry-consistent and chemically semantic coordinate contract
that can then be serialized as Gaussian-readable GICs or consumed directly by
\MATRIX\ modules.
...
semantic coordinate rather than as an opaque handwritten expression.  To the
author's knowledge, no publicly available coordinate generator currently
combines automatic point-group treatment, rank-complete non-redundant selection
and protected semantic coordinates in a single reusable contract.
...
\NEO\ therefore exposes a small semantic front-end rather than a second GIC
language.  A user may request protected base objects, such as
\texttt{DISTANCE}, \texttt{TORSION}, \texttt{RING\_PUCKERING},
\texttt{BUTTERFLY}, fragment translations or center distances, and derived
observables, such as inverse distances, exponentials, trigonometric functions,
linear combinations, Cremer--Pople variables or proton-transfer coordinates.
Protected semantic coordinates enter the same rank and symmetry contract as
automatically generated protected coordinates whenever they are independent.
Observable coordinates never compete in the rank selection; they are evaluated
later as \(y=f(\q)\) from named base coordinates by the chain rule in
Eq.~\eqref{eq:chain-rule}.  Duplicate user-protected rows are rejected, row-equivalent user
coordinates substitute the automatic primitive with provenance recorded, and a
partial user-specified symmetry orbit is diagnosed explicitly before local SALC
fallback.  Thus the user expresses intent, while \NEO\ remains responsible for
model construction, rank resolution and export.
...
The scope of the numerical evidence is stated explicitly in
...
\MATRIX\ development lineage, not a comparison with an external published
...
interface for \NEO, not a competing coordinate-generation theory.  A direct
numerical comparison with explicitly written Gaussian GIC input would still be a
useful cross-method reproducibility check.
...
again at later optimization, scan or refinement geometries.  If \(m\) is the
...
Several validation items also remain intentionally open.  The local-stability
theorem, Theorem~\ref{thm:local-stability}, gives an existence threshold
...
inside a fixed topology/symmetry stratum.  Likewise, the manuscript does not
yet compare \NEO\ against an external published natural-coordinate generator;
the Gaussian tests are export and solver-consumption tests, not an external
coordinate-generator benchmark.  Finally, the protected-prefix rejection rule is
specified and implemented as a fail-fast contract condition, but no numerical
example is shown in which independent protected coordinates exceed the target
rank.  Such a deliberately overconstrained toy state would be a useful
diagnostic example because it would demonstrate that \NEO\ rejects an invalid
semantic contract rather than repairing it by silently deleting protected
coordinates.
...
is closed, pseudo-bonds should be presented as a development target,
...
Finally, determinism is conditional on the frozen \ORACLE/\MATRIX\ state:
...
contract is relevant to force-field and large-amplitude work within the scope
defined above.  Full GF/PED, DVR and \MORPHEUS\ application benchmarks are the
next validation layer.  The cyclohexane export check further shows that a
\NEO{}-generated puckering path can be represented either as ordinary Cartesians or
as Gaussian \texttt{ReadAllGIC} input with numerically identical energies.

\section*{Reproducibility}

The calculations reported in the validation and application sections were
generated with the \MATRIX/\NEO\ development tree and Python 3.13.11.  Gaussian
...
used to generate the tables and application plots.  The \MATRIX/\NEO\ source
code is under active development and will be released with the \MATRIX\
...
Vincenzo Barone conceived the method, designed the \MATRIX/\NEO\ coordinate
contract, implemented the active code paths used here, performed the
calculations and wrote the manuscript.  Planned consumers such as \TRINITY\ and
\SMITH\ are identified as future workflow components rather than as completed
validation targets.

\section*{Acknowledgments}

The author acknowledges the broader \MATRIX\ development environment used to
test the coordinate contracts and the Gaussian GIC work that motivated the
explicit distinction between coordinate-description languages and automatic
