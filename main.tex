...
exponential-map parametrization and retains the relative rigid-body directions
as a protected, typed and symmetry-labelled source family. \\
\midrule
...
Within the \SONIC\ contract, one fragment is selected deterministically as the
reference; three relative translations and, for non-linear partners, three
relative exponential-map orientations are generated for every other fragment.
These rows remain a protected semantic block during rank reduction, transform
explicitly as polar and axial vectors under the stored point-group operations,
and are serialized together with their centers, body frames, gauge choices,
symmetry labels and analytic \(\Bmat\) rows.  Thus the TRIC exponential-map
idea is retained inside the same frozen coordinate definition used by the
other MATRIX modules.
