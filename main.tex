...
Gaussian GIC syntax & Very general coordinate expressions, including
nontraditional geometric parameters, can be evaluated by an external optimizer;
the recent Gaussian implementation is the most direct general-purpose
comparison.\cite{Marenich2025GIC} & The most general coordinates still have to
be specified or selected through input/connectivity rules, and the syntax does
not define a complete natural-coordinate generator with protected rank
reduction, homogeneous point-group projectors and reusable symmetry-labelled
blocks for refinement or force-field analysis. \\
...
\(\phi\) descriptors derived from linear \(RPck\) components.  Recent work by
...
membership and priority order remain fixed.  Assume also that every row norm,
rank residual and projected-vector norm used by the construction has a nonzero
margin from its stored acceptance threshold at \(S_0\).  Then there exists
\(\delta>0\) such that every state \(S\) in this same discrete stratum with
\(\|\x(S)-\x(S_0)\|<\delta\) generates an isomorphic coordinate contract: the
primitive identities, accepted/rejected rank decisions, protected coordinates,
family labels and symmetry labels are unchanged, while \(\Bmat\) rows and
coefficient matrices vary continuously with the geometry.
\end{theorem}
...
the singular configurations excluded by hypothesis.  The row norms,
Gram--Schmidt residuals, block-local Gram matrices and projected-vector norms
are therefore continuous functions of \(\x\).  Since only finitely many such
tests are performed and each has a positive margin from its threshold at
\(S_0\), continuity gives a common neighbourhood in which every inequality used
by the construction has the same truth value.  The deterministic ordering then
forces the same accepted and rejected primitive identities.  Fixed point-group
representations give the same projectors, and the nonzero projected vectors
remain nonzero with the same normalization convention.  Hence the discrete
contract is unchanged up to isomorphism, and the analytic rows and coefficients
vary continuously.
\end{proof}
...
representative cases.  A direct numerical comparison with hand-written Gaussian
GIC definitions is a useful external-facing test, especially for simple
molecules where equivalent coordinate spaces can be built manually.  It is not a
substitute for the \NEO\ contract, because Gaussian GIC syntax does not supply an
automatic general generator, but it should be included before submission in a
final validation set to show that the generated space agrees with an independent
GIC construction when both are available.  Until that comparison is added, the
