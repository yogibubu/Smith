...
Gaussian's internal coordinate builder.  We also discuss the intended scope of
the method and its current limitations.
\end{abstract}
...
The perception layer used here is a standalone implementation of published
chemical descriptors: continuous molecular perception in the spirit of
...
may be selected to make the gauge invariant under isotopic substitution.  At
each electronic-structure call the Cartesian geometry returned by the backend
is aligned to the current MATRIX geometry by a proper Kabsch rotation, and the
gradient (and Cartesian Hessian, when present) is transported by the same
rotation.  Point-group symmetry can therefore remain enabled in the backend:
no assumption is made that its standard orientation agrees with the optimizer
frame.
...
For the water-dimer regression, the original intermolecular construction
required 28 macroiterations and differed from the Gaussian 16 endpoint by
0.07459~\AA{} RMSD; the transported exponential-map implementation requires 20
macroiterations and gives 0.00346~\AA{} RMSD.  A more aggressive chart reset
reaches 0.000246~\AA{} but takes 24 iterations and is therefore retained as a
diagnostic rather than the default.
...
\midrule
Out-of-plane/improper rows & Active & Native \(U\) rows are default; Gaussian-style improper dihedrals are a compatibility mode. \\
\midrule
...
checks.  The native \SONIC\ contract uses signed out-of-plane rows and
block-local multi-dihedral \(RPck\) puckering rows.  Those rows have been
validated with a development Gaussian implementation that supports them as
active variables.  Commercial Gaussian 16 has no native \(U\) out-of-plane
...
\texttt{ReadAllGIC} export therefore uses a compatibility profile only when the
\texttt{g16} option is requested: \(U\) rows are replaced by the corresponding
...
generated coordinate block and reaches a stationary point in two to five
...
Norbornane & \(C_{2v}\) & 51/51 & -- & 2 & \(-268.893823525\) \\
...
In the development Gaussian build, the default internal-coordinate model for
camphor is sensitive to the automatic \(CX_3\) methyl treatment; adding
\texttt{Geom=(GIC,NoXY3)} restores the stable standard-coordinate reference.
...
used to generate the tables and application plots.  The \SMITH\ source code is
being prepared as a standalone distributable package that reads a single
extended-XYZ input file, constructs missing topology and symmetry descriptors
when they are absent, and writes the \SONIC\ contract plus optional Gaussian
\texttt{ReadAllGIC} input.  Until public release, the code and frozen
input/output data are available from the author upon reasonable request.
