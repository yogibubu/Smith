...
\paragraph{Construction principle: deterministic contract.}
...
stored contract and sparse \(\Bmat\) rows; the GF layer then applies
...
geometries or GIC serializations.  It confirms that the contract is
...
stored contract with primitive definitions, coefficients, family
...
contract is constructed once for a frozen molecular state; only the
...
near-threshold operations in the stored contract; quantitative
...
consumers are discussed only as downstream uses of the contract.

\section*{Acknowledgments}

The author acknowledges the broader \ORACLE\ development environment used to
test the contracts, the Gaussian GIC work that motivated the
