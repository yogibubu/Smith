...
coordinates.  The \TheONE\ implementation starts
...
synthon and symmetry sections from Cartesian coordinates using the bundled
MATRIX tools.
...
The coefficient pattern follows the natural-coordinate construction used by
Pulay and co-workers for ring deformations.  If \(s_j\) denotes a cyclic source
list, such as the endocyclic valence angles or a torsional puckering list, the
stored rows are real Fourier-like combinations
\[
...
optimization choices on two small systems for which a hand Z matrix is
available or unambiguous.  These calculations are not a benchmark of electronic
...
coordinates and in five steps with the hand Z matrix.
...
coordinates.  The water and hydrogen-peroxide tests are deliberately small:
their role is to verify solver consumption and input portability, not to rank
coordinate strategies statistically.}
