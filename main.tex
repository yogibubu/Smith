...
\begin{abstract}
Internal coordinates are the natural language of molecular structure, yet
automatic constructions either retain redundancy or produce delocalized,
weight-dependent bases and remain difficult to generalize to fused and bridged
rings, high coordination, exact and local symmetry, and weakly bound
fragments.  We introduce \SONIC\ (Symmetry-Oriented Non-redundant Internal
Coordinates), implemented in \SMITH\ (Symmetry-Mapped Internal-coordinate
Template Handler), as a deterministic protocol that accepts a frozen molecular
state or provider-neutral Cartesian, topology, or primitive-coordinate input;
organizes ordinary and special coordinates in chemically homogeneous families;
protects ring and fragment semantics; and applies analytic Wilson
\(\Bmat\)-row rank tests followed by local and molecular symmetry projection.
...
rotation/permutation covariance, stable local symmetry-adapted combinations,
and reproducible external serialization.  Gaussian 16 is used as an independent
parser and solver, with SONIC serialization representing one particular use of
its much more general GIC syntax.
The resulting frozen contract retains chemically readable decomposition,
symmetry, rank diagnostics, analytic first derivatives, and provenance, thereby
providing a constructive counterpart to natural internal coordinates while
leaving optimization, finite internal-to-Cartesian realization, force fields,
and higher derivatives to downstream consumers.
\end{abstract}
...
For repeated automatic workflows, molecule-specific GIC definitions for local
\(XY_n\)-type combinations, proton-transfer sums and differences, fragment
translations or ring-puckering components can instead be generated from the
frozen molecular state.  When those variables are consequences
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Targeted tests inspired by the Gaussian 16 GIC Supporting Information.
The purpose is to test automatic \SONIC\ construction of selected coordinate
objects that Gaussian 16 can evaluate when supplied as GIC expressions or
through dedicated options; these objects represent only a subset of the uses
supported by the general GIC syntax.}
\label{tab:frisch-si}
...
used the commercial Gaussian 16 Rev. C.01 installation at HF/STO-3G.  No feature
