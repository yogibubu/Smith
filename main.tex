...
labels are unchanged.  \NEO\ therefore treats pivot changes as path diagnostics:
the stored overlap between consecutive SALC coefficient matrices is monitored,
and a continuity-preserving Procrustes alignment can be applied by downstream
GF/PED or DVR layers when a path-continuous basis is required.  None of the
reported chair-to-boat, ferrocene or GF/PED probes showed a pivot switch in the
active blocks used for the tabulated quantities.
...
frozen strata.  The practical response is to record the perception margins and
to flag near-threshold symmetry changes; quantitative tolerance-sensitivity
