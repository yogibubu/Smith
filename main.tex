...
active variables.  Commercial Gaussian 16 has a documented limitation for
active generic GICs that combine multiple periodic dihedrals, and it has no
native \(U\) out-of-plane primitive.  The commercial \texttt{ReadAllGIC}
export therefore uses a compatibility profile: \(U\) rows are replaced by the
corresponding Gaussian improper dihedrals, multi-dihedral \SONIC\ rows are kept
as inactive diagnostics, and all component \(D(\cdots)\) terms are added as
active redundant periodic coordinates.  After the Gaussian optimization, the
optimized geometry is evaluated again in the frozen non-redundant \SONIC\
contract.  The calculations reported here used Gaussian 16 at HF/STO-3G.
...
redundant internals, 14 Cartesian steps, nine \SONIC\ compatibility steps and 33 steps for a
...
Norbornane & \(C_{2v}\) & 51/51 & 4 & 4 & 6 & 9 \\
Camphor & \(C_1\) & 75/75 & 8 & 14 & 33 & 9 \\
\bottomrule
\end{tabular}
\end{table}
...
coordinates, while the original \(RPck\) combinations are written as inactive
diagnostics.  The optimized structure is then projected back onto the native
\SONIC\ contract.  This compatibility export is less compact than the native
