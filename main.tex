...
Sparse Wilson-\(\Bmat\) storage & Active runtime baseline & The Gaussian GIC
implementation stores derivative rows sparsely, and \NEO\ now exposes sparse
Wilson-row objects for iterative evaluation.  Dense matrices remain available
for compact rank and projector audits. \\
Canonical primitive/function registry & Active baseline & The Gaussian
...
generated primitives and \(\y=f(\q)\) functions, preserving the generator
contract while preventing duplicated coordinate objects. \\
Coordinate-function layer & Active baseline & Inverse and exponential
...
cross-program reproducibility check rather than the first evidence that the
