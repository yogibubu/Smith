...
templates before block-local reduction.  In the optional local-pseudosymmetry
...
torsions remain separate local source blocks, and the cyclic operations used
...
tolerances, not physical constants; both are written to the diagnostic record.
...
template is accepted, the construction falls back to ligand-equivalence
classes without changing the final rank criterion.
Angles are partitioned separately according to the pair of ligand classes, so
...
ring stretches, endocyclic valence angles and ring torsions are transformed in
...
dihedrals are averaged as one local-rotor \(A_1\) orbit; if the orbit cannot be
closed reliably, the existing one-dihedral rule is used and the fallback is
recorded.  Every generated row still passes the final analytic
\(\Bmat\)-rank test.
...
\midrule
High coordination & Active & Coordination 5--9 by two templates per coordination number or ligand-equivalence fallback; canonical local SALCs are implemented in both Python and Fortran. \\
\midrule
...
coordination number from five to nine, while retaining the
ligand-equivalence fallback for unrecognized or chemically split environments.
...
distance tolerances, all four final ranks, the rigid-rotation check and the
