...
\title{\SONIC\ (Symmetry-Oriented Natural Internal Coordinates): Block-Local, Symmetry-Consistent Generalized Internal Coordinates for Molecular and Intermolecular Coordinate Spaces}
...
are GDV/post-processing and DVR variables, not commercial Gaussian 16
\texttt{ReadAllGIC} rows. \\
...
three to nine steps for saturated ring and cage systems with different symmetry and
rank.
...
iteration count.  They test the narrower point that \SMITH\ can generate a
...
redundant internals, 14 Cartesian steps, 20 \SONIC\ steps and 33 steps for a
...
A separate Gaussian DV check was carried out with the native \(U\)
out-of-plane export enabled, rather than replacing \(U\) rows by
Gaussian-compatible improper dihedrals.  In this mode camphor contains one
native \(U(6,4,7,8)\) row and converges in eight HF/STO-3G optimization steps.
The commercial Gaussian 16 compatibility input uses active linear \(RPck\)
rows for ring puckering and the Fortran/G16 improper-dihedral convention
\(U(I,J,K,L)\mapsto D(J,I,L,K)\), giving \(D(4,6,8,7)\) for the same local
non-planarity.  Polar \(Q/\phi\) puckering functions and native \(U\) rows are
therefore reserved for the Gaussian DV profile.
...
These estimates describe the implemented algorithms, not a timed benchmark.
The present evidence validates rank closure, sparse-row construction and
external solver consumption on representative systems; it does not yet provide
scaling curves versus \(N\), ring complexity, point-group order or fragment
count.  A release-quality performance table should therefore report
construction time, per-geometry \(\Bmat\) time, sparse density, peak memory and
projector time for controlled molecular series.
