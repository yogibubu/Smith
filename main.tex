...
\newcommand{\TRINITY}{TRINITY}
\newcommand{\SMITH}{SMITH}
\newcommand{\SYCART}{SYCART}
\newcommand{\GIC}{GIC}
...
engine of \MATRIX\ (Molecular Analysis Toolkit for Reusable Integrated
eXperiments), as a general construction of non-redundant,
...
same frozen definition is designed to supply Gaussian-readable coordinates, analytic
...
sparse derivative machinery and Gaussian export path; production GF/PED, DVR
and semi-experimental refinement benchmarks are downstream applications rather
than claims of the present validation set.
\end{abstract}
...
The acronyms used below denote roles in this workflow.  \MATRIX\ is the
Molecular Analysis Toolkit for Reusable Integrated eXperiments, the shared
molecular-state and package framework.  \ORACLE\ is the Operator for Routing,
Analysis, Control, Launch and Exploration; in this manuscript the relevant
\ORACLE/\MATRIX\ object is the validated molecular state, including continuous
atom perception and synthon descriptors used by \NEO\ for deterministic local
choices.  \SYCART\ denotes the symmetry-adapted Cartesian active-space model
used as a Cartesian comparison basis.  \MORPHEUS\ is used as a tool name for
SEFit, the semi-experimental fit module; \TRINITY\ is the planned external-gradient
optimization branch, and \SMITH\ (Surface Modeling with Internal-coordinate
Terms and Hamiltonians) is the planned multidimensional PES/property-surface
fitting layer, with PES denoting potential-energy surface.  GF/PED denotes
Wilson GF analysis and potential-energy distribution, and DVR denotes discrete
variable representation.

The same separation makes \NEO\ a coordinate service rather than a single
application module.  GF/PED is the immediate spectroscopic consumer of the
generated coordinates for Wilson analysis, vibrational scaling, local
force-field analysis and force-constant models.  The DVR layer uses the same
coordinate state to choose large-amplitude coordinates, build ring-puckering
grids and associate grid points with molecular geometries.  \TRINITY\ is the
future external-gradient optimization consumer of the same \GIC/\SYCART\ state.
\SMITH\ is the planned consumer for multidimensional potential-energy and
property-surface fitting, while \MORPHEUS\ uses the frozen basis for one- and
multidimensional semi-experimental fits.  In all cases the
...
Initial local equivalence is not based only on element symbols.  The
\ORACLE/\MATRIX\ molecular-state layer supplies continuous atom perception and
...
variables in GF/PED, DVR, future \TRINITY\ and \SMITH\ workflows, or
\MORPHEUS.
...
The tuple is the object consumed by GF/PED, DVR, \TRINITY, \SMITH\ and
\MORPHEUS.
...
orbits and only when the \ORACLE/\MATRIX\ continuous bond-order and synthon
...
a family are resolved by deterministic topology, \ORACLE/\MATRIX\ equivalence and
...
preserves element identities, geometry, topology, \ORACLE/\MATRIX\ descriptors, fragments,
...
For a fixed molecular state--geometry, topology, \ORACLE/\MATRIX\ descriptors, point-group
...
interaction centers and \ORACLE/\MATRIX\ equivalence descriptors.  Candidate ordering is
...
neighbourhood of \(S_0\), topology, fragment decomposition, \ORACLE/\MATRIX\ equivalence
...
rank.  The current Python construction and the archived Merlino/Fortran
construction path must agree on coordinate families, final rank and
...
\MORPHEUS\ refinement, but those are downstream application validations.  The
present paper validates the frozen coordinate contract itself and then tests
whether external Gaussian calculations can consume NEO-generated coordinates.
It therefore shows that the geometric and algebraic object is well defined; it
does not claim a production benchmark of GF/PED scaling, DVR dynamics or
semi-experimental refinement.
...
archived Merlino construction path; the reported residual is the
Wilson-row-space residual between two internally maintained non-redundant bases
that share the same scientific lineage but not the same implementation path.
The narrow range of
...
representation, not to benchmark an electronic-structure method or to replace
full GF/PED, DVR and \MORPHEUS\ validation studies.
...
This is still a solver and coordinate-contract check, not an electronic
structure benchmark.  The relevant observations are that Gaussian accepts the
...
is not that \NEO\ constructs normal modes, nor that one small molecule proves a
general vibrational benchmark.  The figure shows the diagnostic that will be
applied across a broader corpus.  It is therefore an illustrative locality
check: in this case, a chemically labelled basis makes the Hessian visibly more
block structured before any empirical scaling or normal-mode diagonalization is
applied.
...
The final probe tests a stricter export question.  \NEO\ was used to construct a
fixed-frame cyclohexane puckering path from the chair \(q_3\) component to the
boat \(q_2\) component.  For each point it generated two independent external
representations of the same geometry: a portable Cartesian input, usable by any
electronic-structure program, and a Gaussian \texttt{ReadAllGIC} input containing
the automatically generated coordinate contract.  Gaussian DV B3LYP/6-31+G*
single-point energies were then evaluated from both inputs.
...
symmetry-projector normalization and Python--Fortran/Merlino row-space
equivalence for representative cases.  This is an implementation-level
cross-check inside the \MATRIX\ lineage, not a claim of agreement with an
external natural-coordinate program.  Table~\ref{tab:frisch-si} adds the more external-facing
...
\NEO\ model can generate these variables.  By contrast, production GF/PED,
DVR and \MORPHEUS\ examples remain outside the validation set of this paper:
the present figures show coordinate locality, large-amplitude export and
Gaussian consumption, not a completed spectroscopy or refinement benchmark.
...
puckering and portable chair-to-boat export probes illustrate why the same
contract is relevant to force-field and large-amplitude work, but they remain
coordinate diagnostics.  Full GF/PED, DVR and \MORPHEUS\ application benchmarks
are the next validation layer.  The cyclohexane export check further shows that
a NEO-generated puckering path can be represented either as ordinary Cartesians
or as Gaussian \texttt{ReadAllGIC} input with numerically identical energies.
