...
construction.  The table is included in the draft because it prevents a common
manuscript error: describing intended extensions as if they were already part of
the release-grade coordinate contract.

\begin{longtable}{p{0.27\linewidth}p{0.18\linewidth}p{0.45\linewidth}}
\caption{Current status of the main \NEO\ coordinate features.}
\label{tab:status}\\
\toprule
...
Sparse Wilson-\(\Bmat\) storage & Planned optimization & The Gaussian GIC
implementation stores derivative rows sparsely, which is the right
representation for local primitive rows and fragment/function coordinates.
\NEO\ should retain sparse row objects through contract evaluation and reserve
dense matrices for compact rank and projector audits. \\
Canonical primitive/function trees & Planned optimization & The Gaussian
Supporting Information uses search trees to avoid duplicate primitive records
in large GIC expressions.  A hash/tree or expression-DAG registry would give
\NEO\ the same protection for automatically generated primitives and
\(\y=f(\q)\) functions in large systems. \\
\bottomrule
\end{longtable}
...
this draft.  The first eight rows were generated with
\texttt{matrix gicforge fortran-audit} against the vendored Merlino backend; the
reported residual is the Wilson-row-space residual between the Python and
Fortran non-redundant bases.  The narrow range of
...
backends saturate the same numerical equivalence criterion across systems of
different size and symmetry.  The formic-acid--water row uses the release-grade
\texttt{SPECIAL\_COORDINATES} fragment path, for which the legacy Fortran
comparison is not the relevant reference; there the table reports direct
...
representative cases.  A direct numerical comparison with Gaussian GIC
definitions should still be included before submission, but its purpose should
be precise.  The useful examples are not generic bond lengths or angles; they
are the Supporting-Information cases where chemical meaning enters through a
written expression or a special option: the formamide \(\Sigma r/\Delta r\)
transfer variables, \(XY_3\)-symmetrized local blocks, Li\(_2\)--Ar polar or
elliptic scan functions, benzene-dimer/water-cluster rotor coordinates and
ferrocene quaternion coordinates.\cite{Marenich2025GIC}  Showing that \NEO\
can generate the corresponding base or protected coordinate objects, and that
Gaussian can reproduce them when the GIC input is written explicitly, would
make the complementarity visible: Gaussian supplies a powerful expression and
derivative language, while \NEO\ supplies the automatic construction and frozen
provenance of the coordinate contract.  Until that comparison is added, the
validation claim is limited to internal rank closure, symmetry consistency,
SYCART active-space agreement and Python/Fortran row-space equivalence.

The same comparison also suggests two implementation improvements.  First,
contract rows should be stored and propagated sparsely wherever possible,
because distance, angle, torsion, center and fragment rows have small local
support even when the final audit is performed in the full Cartesian space.
Second, generated primitives and derived functions should be registered through
a canonical hash/tree or expression-DAG structure, analogous in purpose to the
primitive search trees described for Gaussian GICs.\cite{Marenich2025GIC}
That would prevent duplicated primitive rows in large automatically generated
contracts and would make the future \(\y=f(\q)\) layer scalable.
