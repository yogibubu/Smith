...
Name & Role \\
\midrule
\MATRIX & Molecular Analysis Toolkit for Reusable Integrated eXperiments; the
shared molecular-state and package framework. \\
\ORACLE & Operator for Routing, Analysis, Control, Launch and Exploration; here,
the validated state carrying continuous atom perception and synthon descriptors. \\
\NEO & Coordinate-generation engine: \GIC/\SYCART\ construction, analytic
\(\Bmat\) rows, symmetry labels and provenance. \\
\SYCART & Symmetry-adapted Cartesian active-space model used as an independent
Cartesian comparison basis. \\
\MORPHEUS & SEFit semi-experimental refinement module using \NEO{} coordinates. \\
\TRINITY & Planned external-gradient optimization branch. \\
\SMITH & Planned Surface Modeling with Internal-coordinate Terms and Hamiltonians
layer for multidimensional PES/property fitting. \\
...
Gaussian-readable GICs or consumed directly by \MATRIX\ modules.
...
\begin{proposition}[Rank-preserving selection]
...
Semantic \texttt{PROTECT}/\texttt{OBSERVABLE} front-end & Active baseline &
...
SALC blocks, not as a symmetry-adaptation test.  The semantic-front-end row is
...
Semantic front-end & \texttt{PROTECT}/\texttt{OBSERVABLE} contract & -- & unit probes & duplicate protected row rejected; row-equivalent substitution recorded; protected rank excess fails fast \\
\bottomrule
\end{tabular}
\end{table}
...
coordinate-generator benchmark.  The protected-prefix rejection rule is covered
in the semantic front-end unit probes in Table~\ref{tab:validation}; a chemically
realistic overconstrained example remains a useful future diagnostic.
...
\paragraph{Proof of Theorem~\ref{thm:relabel}.}
...
\paragraph{Proof of Proposition~\ref{prop:determinism}.}
...
interaction centers and \ORACLE/\MATRIX\ equivalence descriptors.  Candidate
ordering is the fixed total order extending the family preorder and local
tie-breaking keys.  Rank decisions are Boolean tests of normalized analytic
Wilson-row residuals against stored tolerances.  Symmetry adaptation is
performed block by block from the stored permutation representation and a fixed
normalization/sign convention for the projected coefficient vectors.  Composing
these maps gives either one contract or one recorded incompatibility certificate.

\paragraph{Proof of Proposition~\ref{prop:protected}.}
...
\paragraph{Proof of Proposition~\ref{prop:rank-selection}.}
...
\paragraph{Proof of Theorem~\ref{thm:block-scaling}.}
...
\paragraph{Proof of Theorem~\ref{thm:local-stability}.}
