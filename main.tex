...
ordered redundant primitive/B source space.  In an integrated MATRIX workflow
the complete state is supplied by ORACLE; standalone \SMITH\ can instead consume
user-supplied topology or primitive definitions, or construct the minimal
topology and ordinary primitives required by \SONIC\ from Cartesian input.  It validates and classifies
...
Name & Role \\
\midrule
ORACLE molecular-state layer (integrated workflow) & Input/perception layer that either reads saved topology,
...
minimal frontend.  It performs SONIC construction, special/composite coordinate generation, symmetry-adapted
...
\node[stage=blue!7] (state) {Molecular state\\or standalone\\topology};
...
starting from a molecular-state and redundant primitive/B contract.  The
integrated route may consume the full ORACLE state, whereas standalone SMITH
can construct or accept the topology/primitive boundary directly.
...
state.  In the integrated MATRIX workflow their full definition belongs to
ORACLE (Operator for Recognition,
...
operations and atom permutations, atom equivalence, effective atomic numbers,
the charge, covalency, delocalization and strain, the topological edge order,
the distinct Mayer/Pauling multiplicity, and separate
$\sigma$, first-$\pi$ and second-$\pi$ ($\pi\pi$) synthon indices used below.
\SMITH\ consumes these objects when present and does not recompute them.  The
complete state also supplies the ordinary stretch, bend, linear-bend, dihedral
...
For standalone use the required boundary can be supplied in a single
...
X--H policies and an optional reduced working subgroup.  Four input profiles
are accepted.  A complete frozen state is consumed unchanged; a supplied
\texttt{TOPOLOGY} section is used to generate the ordinary redundant
primitives; a supplied \texttt{PRIMITIVES} section is validated against the
Cartesian geometry and used directly; and a plain XYZ invokes a bundled
minimal frontend that constructs both topology and ordinary primitives.  If
topology and primitives are both supplied, their bond rows must agree.  The
output records the selected profile as \texttt{FROZEN\_STATE},
\texttt{STANDALONE\_TOPOLOGY}, \texttt{STANDALONE\_PRIMITIVES} or
\texttt{STANDALONE\_MINIMAL}.  The minimal route is sufficient for ordinary
molecules and the supplied examples; advanced continuous descriptors,
interaction centers or externally validated symmetry can instead be supplied
through the complete-state interface.  Thus SMITH is operationally standalone
without erasing the scientific boundary between perception and coordinate
construction.
...
$\max(\mathrm{BO}-1,0)$ used historically for all multiple bonds.
...
dihedral is never selected during ORACLE perception or SMITH reduction.  Only
...
\subsection{Finite predictor--constrained corrector realization}
The Wilson right inverse remains appropriate for infinitesimal motion and for
the symmetric display vectors discussed below, but it is not used as the
primary generator of a large soft-coordinate displacement.  The nonlinear
realization service distributed with the \SONIC\ contract instead uses a
predictor--corrector construction.  Acyclic one-dihedral coordinates are
recognized by removing their central bond from the frozen graph.  When this
separates the graph, one component is rotated rigidly through the requested
finite angle,
\begin{equation}
 \mathbf x_i'=\mathbf a+
 \exp\!\left([\Delta\phi\,\hat{\mathbf u}]_\times\right)
 (\mathbf x_i-\mathbf a),
\end{equation}
which is the Rodrigues rotation written as an exponential map.  Complete
interfragment translation and orientation triplets are imposed as rigid-body
motions; rotation increments are composed on \(\mathrm{SO}(3)\), rather than
added as large rotation vectors.

After this finite predictor, stretches, bends and the other hard targets are
restored by damped least squares in the numerical null space of the protected
soft rows.  If \(B_h\) and \(B_s\) denote the current hard and protected-soft
Wilson blocks and \(Z_s\) spans \(\ker B_s\), the corrector solves
\begin{equation}
 \min_{\mathbf y}
 \left\|B_hZ_s\mathbf y-\mathbf r_h\right\|^2
 +\lambda\|\mathbf y\|^2,
 \qquad \Delta\mathbf x=Z_s\mathbf y .
\end{equation}
The finite predictor is reapplied after each nonlinear correction, so the hard
projection cannot systematically retract the requested torsion or fragment
pose.

A single bond rotation is not a valid finite predictor for a cyclic bond.
Ring-puckering, butterfly, inversion, mixed torsional SALCs and other coupled
soft coordinates therefore follow a continuation path.  Each path increment
is corrected before the next is taken, so ring closure and the selected hard
coordinates are enforced throughout.  The default maximum increment is
0.12 rad and is reduced effectively by adding substeps near linear geometries;
both this value and the maximum number of substeps are exposed by LINK and its
GUI.  Thus scan points and SENTINEL candidates use the same frozen \SONIC
semantics, while choosing the appropriate finite or continuation realization
from the coordinate topology.

For PES exploration the equilibrium SONIC contract is not blindly reused with
its reference point-group adaptation.  SMITH constructs a transient basis in a
user-declared retained group, understood as the symmetry common to the entire
scan or population and defaulted to \(C_1\).  ORACLE may still detect and
project a different instantaneous Cartesian point group at every geometry for
the subsequent energy calculation; that operation never relabels the frozen
exploration variables.  Exocyclic torsions used as scan genes remain single
physical dihedrals and are excluded from both local and retained-group SALC
combination.
...
\midrule
Sparse first derivative of \(\Bmat\) & Active baseline & Analytic primitive Hessians are assembled into Cartesian slices of $\partial B_{ia}/\partial x_b$ and stored as sparse row maps; dense three-index tensors are audit-only. \\
\midrule
...
The first Wilson-matrix derivative is represented as
\begin{equation}
 \mathcal D_{iab}=\frac{\partial B_{ia}}{\partial x_b}.
 \label{eq:sparse-b-first-derivative}
\end{equation}
The production implementation evaluates the first Wilson-matrix derivative
analytically by applying second-order forward-mode automatic differentiation
to each local primitive.  Equivalently, the Cartesian Hessian of every
supported primitive is obtained by exact chain-rule propagation, without
finite differences.  The frozen
linear coefficients defining a PIC or SONIC then assemble these primitive
Hessians directly into $\partial\Bmat/\partial x_b$ slices, each retaining only
its nonzero row--column map.  A parallel fourth-order centered finite-difference
evaluation of the analytic $\Bmat$ rows is retained only as an independent audit
and as an explicitly reported fallback at a singular or unsupported primitive;
it is not the production route. Consequently neither PIC nor SONIC use a dense
$n_q\times(3N)^2$ object. At a
nonstationary geometry the curvature contribution required by the Hessian
chain rule is accumulated directly from the sparse slices,
\begin{equation}
...
trajectory on which Cartesian and internal Hessians are exchanged.  The same frozen-coordinate API is
used for primitive PIC rows and for their local SONIC combinations.  The
analytic path covers stretches, valence and linear bends, dihedrals,
out-of-plane coordinates, ring combinations, fragment translations and
finite fragment rotations; the contract records the primitive identifiers of
any numerical fallback.
...
$\eta^3$ allyl--palladium complex tests an ORACLE-supplied ligand center, and
...
special coordinates; formic acid--water tests relative fragment coordinates;
and $\eta^3$ allyl--PdCl tests a protected multi-atom ligand center.  Dashed
turquoise segments identify intermolecular contacts or a protected
center--metal coordinate, and the turquoise marker denotes the ORACLE-supplied
$\eta^3$ center.  The cyclohexane panels show three points along the chair-to-boat
puckering path used for the portable Cartesian/Gaussian \texttt{ReadAllGIC}
comparison.}
...
The stored first derivative of the Wilson matrix completes this audit at
second order.  The ordinary Wilson matrix describes the tangent map
...
\eqref{eq:hessian-back-transformation} separate physical force-constant
curvature from coordinate curvature instead of folding the latter into an
apparent Hessian.  This distinction is immaterial for an exactly stationary
harmonic GF calculation, where the gradient contraction vanishes, but it is
essential for constrained stationary points, scans, partial optimizations and
Hessian transport along a path.  Because SONIC rows are frozen linear
combinations of typed source rows, their \(\Bmat'\) slices are assembled with
the same coefficients; no dense three-index tensor or numerical refit is
introduced.
...
The corresponding \(\Bmat'\) service is evaluated only when a second-order
transformation is requested.  Its work follows the stored nonzero entries of
the participating primitive Hessians and the contraction in
Eq.~\eqref{eq:sparse-b-curvature}; Table~\ref{tab:construction-cost} times
\(\Bmat\), not this optional second-order service, and should not be read as a
benchmark of \(\Bmat'\).
...
The same locality qualification applies to \(\Bmat'\).  Analytic second
derivatives are available for every supported primitive and are propagated to
PIC and SONIC rows by the exact chain rule.  Singular or unsupported primitive
charts trigger an explicitly reported numerical fallback; they are never
silently mixed into the analytic path.  Near linear-angle singularities,
fragment-rotation chart boundaries or a topology change, the appropriate
remedy is to switch or rebase the coordinate chart and rebuild the contract,
not to interpret a divergent \(\Bmat'\) as physical curvature.
...
bundled minimal Cartesian frontend provides only the information needed to
construct ordinary SONIC coordinates.  Results that depend on advanced
continuous atom equivalence, externally validated point-group operations or
special interaction centers require those data in the input state.  This is a
capability distinction between input profiles, not a runtime dependency of
SMITH on a separate application.
...
Starting from a frozen topology and redundant primitive/B contract, \SONIC\ provides a general, frozen and symmetry-aware construction of
...
frozen code state is MATRIX tag \texttt{smith-v0.1.0rc5}, commit
\texttt{4003cd7b8607446036134ca52386397138a9b957}; the corresponding SMITH
manuscript tag and checksums are recorded with the archived submission.  Gaussian
...
plain Cartesian input through the bundled minimal frontend.  The provider-neutral
\texttt{--require-frozen-state} option requires the complete contract.  SMITH, including optional Gaussian
\texttt{ReadAllGIC} export, is developed at
\url{https://github.com/yogibubu/MATRIX}.  The package pins only reduced-perception
components; a complete ORACLE state is one optional integrated provider and is
described in its companion manuscript.
The reproducible standalone environment is tagged \texttt{v0.1.0rc5} in the
SMITH repository and pins the exact MATRIX commit stated above. Repository
revisions and \texttt{\#SMITH\_PROVENANCE} record the code and
perception profile.
