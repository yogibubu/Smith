...
models.  The DVR branch uses the same coordinate state to choose large-amplitude
...
paper is not that a new Wilson matrix is needed.  It is that a production
...
coordinate contract: a persistent mathematical object that downstream programs
...
deterministic local choices.  The current runtime uses effective atomic numbers
...
classes.  In the present implementation, this continuous perception enters ring
...
to this workflow.  In the DVR branch, a one- or multidimensional grid in
...
\NEO\ basis, but they are natural consumers of it.  A production DVR can then
...
The current \NEO\ implementation already introduces bond-order-dependent
...
CIP-like priority keys stored in \(\mathcal{M}\).  Thus the algorithm works on a
...
failure diagnostic.  In particular, no stochastic search, optimizer history or
manual branch choice is part of the contract definition.
\end{theorem}
...
contract or one recorded failure state.
\end{proof}
...
more independent rows than the target rank, the algorithm has no
...
The implementation is the finite-precision version of this statement.  \NEO\
...
vibrational rank, the build stops rather than silently discarding a special
coordinate.

\begin{proposition}[Block-local scaling invariance]
\label{prop:block-scaling}
...
Determinism follows from the frozen state and the stored total order: the same
geometry, topology, ORACLE equivalence data, priority keys and tolerances give
the same contract.  Stability is local rather than absolute.  If a small
geometric perturbation preserves the topology, equivalence classes, family order
and a finite gap between accepted residuals and the rank threshold, the selected
basis is unchanged.  A change in topology, symmetry, local equivalence or a
near-threshold residual is a genuine model change; it is therefore reported as
contract provenance rather than hidden behind a re-diagonalized global basis.
...
Table~\ref{tab:validation} gives the current self-consistency slice used for
...
Fortran non-redundant bases.  The formic-acid--water row uses the production
...
\caption{Representative internal validation of the current \NEO\ implementation.
...
Theorems~\ref{thm:determinism} and~\ref{thm:protected} make the two distinctive
claims precise: the same frozen molecular state defines one coordinate contract,
and protected semantic coordinates are retained whenever rank compatibility
allows them.
