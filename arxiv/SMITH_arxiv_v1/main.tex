...
and the other continuous atomic and synthon descriptors.  It develops Proxima
...
substitute for the full ORACLE application, which remains in final validation
and will be released when ready.  Production calculations can
...
For every final SONIC, SMITH can also generate a pair of linearized Cartesian
motion frames with the Wilson matrix frozen at the reference geometry,
\(\Delta\mathbf{x}_k=\mathbf{B}_0^+\mathbf{e}_k\Delta q_k\) and
\(\mathbf{x}_{k,\pm}=\mathbf{x}_0\pm\Delta\mathbf{x}_k\).  These frames are
exactly symmetric and freeze all other SONICs to first order; they are
visualization vectors rather than nonlinear back-transformations.  An optional
mass-weighted right inverse
\(\mathbf{M}^{-1/2}(\mathbf{B}_0\mathbf{M}^{-1/2})^+\) changes the Cartesian
minimum-norm metric without changing that contract.  The mathematical step is
kept separate from the viewer amplitude, which normalizes each half-vector to a
selected maximum atomic displacement.  Per-coordinate records include
conditioning, amplification, participation ratio and dominant atoms; ring
records additionally preserve the ordered ring and primitive coefficients for
highlighted visualization.  The regression gate generates and consumes all 114
pagodane and all 51 indane motions, including their XYZ and vector files.
...
components; full ORACLE is described in its companion manuscript, remains in
final validation, and will be released when ready.
