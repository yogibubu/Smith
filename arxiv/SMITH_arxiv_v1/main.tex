...
\end{equation}
The production implementation evaluates the Cartesian Hessian of each local
primitive by second-order forward algorithmic differentiation.  The frozen
...
its nonzero row--column map.  A parallel fourth-order centered differentiation
of the analytic $\Bmat$ rows is retained as an independent audit and is invoked
explicitly as a reported fallback at a singular or unsupported primitive; it
is not the production route. Consequently neither PIC nor SONIC use a dense
